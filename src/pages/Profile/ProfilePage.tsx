...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-extrabold text-[var(--color-text-heading)]">My Profile</h1>
      </div>
      ...
      {user ? (
        <div className="max-w-[600px]">
          <div className="bg-gradient-to-br from-[var(--color-bg-card)] to-[var(--color-bg-elevated)] rounded-xl p-6 flex items-center gap-5 mb-8">
            <div className="w-[72px] h-[72px] bg-[var(--color-primary)] rounded-full flex items-center justify-center text-2xl font-extrabold text-white flex-shrink-0">
              {user.name?.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1">
              <h2 className="text-xl font-bold text-[var(--color-text-heading)] mb-0.5">{user.name}</h2>
