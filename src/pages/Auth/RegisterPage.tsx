    ...
    <div className="min-h-[calc(100vh-72px-200px)] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-[420px] bg-[--color-bg-card] border border-[--color-border] rounded-xl p-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-extrabold text-[--color-text-heading] mb-1">Create Account</h1>
          <p className="text-[--color-text-muted]">Join CineBook and start booking</p>
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
          <div className="flex flex-col gap-2">
            <label htmlFor="name" className="text-sm font-semibold text-[--color-text]">Full Name</label>
            <input
              id="name"
              name="name"
              type="text"
              ...
              className={`px-4 py-3 bg-[--color-bg-elevated] border rounded-lg text-[--color-text-heading] text-base transition-colors duration-150 outline-none ${
                errors.name ? 'border-[--color-error]' : 'border-[--color-border] focus:border-[--color-primary]'
              }`}
              placeholder="John Doe"
              aria-describedby={errors.name ? 'name-error' : undefined}
            />
            {errors.name && (
              <span id="name-error" className="text-[13px] text-[--color-error]">{errors.name}</span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="confirmPassword" className="text-sm font-semibold text-[--color-text]">Confirm Password</label>
            <input
              id="confirmPassword"
              ...
              onChange={handleChange}
              className={`px-4 py-3 bg-[--color-bg-elevated] border rounded-lg text-[--color-text-heading] text-base transition-colors duration-150 outline-none ${
                errors.confirmPassword ? 'border-[--color-error]' : 'border-[--color-border] focus:border-[--color-primary]'
              }`}
              placeholder="Repeat your password"
              aria-describedby={errors.confirmPassword ? 'confirm-error' : undefined}
            />
            {errors.confirmPassword && (
              <span id="confirm-error" className="text-[13px] text-[--color-error]">{errors.confirmPassword}</span>
            )}
          </div>
        ...
        <p className="text-center mt-6 text-[--color-text-muted] text-[15px]">
          ...
          <Link to={ROUTES.LOGIN} className="text-[--color-primary] font-semibold hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
